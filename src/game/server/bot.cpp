...
void CBot::Tick()
{
	...
	if (m_pPlayer->GetCharacter()->CanFire())
	...
	if (!SeeTarget)
	{
		...
		if (!m_InputData.m_Fire && m_LastData.m_Hook == 0 && pMe->m_HookState == HOOK_IDLE && (rand() % 10 == 0 || (CurTile & BTILE_HOLE && rand() % 4 == 0)))
		{
			...
			for (int i = 0; i < NumDir; i++)
			{
				...
				if ((Collision()->FastIntersectLine(pMe->m_Pos, Pos, &Pos, 0) & (1 | 4)) == 1)
				{
...
void CBot::HandleWeapon(bool SeeTarget)
{
	...
	for (int c = 0; c < Count; c++)
	{
		...
		else if (pMe->GetWeaponAmmo(WEAPON_RIFLE) != 0 && ClosestRange < GameServer()->Tuning()->m_LaserReach && !Collision()->FastIntersectLine(Pos, apTarget[c]->m_Pos, 0, 0))
		{
			Weapon = WEAPON_RIFLE;
			break;
		}
	...
	if (Weapon < 0)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			if (!pMe->GetWeaponAmmo(Weapons[j]))
				continue;
			...
			for (int k = 0; k < NbLoops && GoodDir == -1; k++)
			{
				for (int i = 0; i < BOT_HOOK_DIRS; i++)
				{
					...
					for (int c = 0; c < Count; c++)
					{
						vec2 InterPos = closest_point_on_line_s(aProjectilePos[i], NextPos, aTargetPos[c]);
	...
	else if (pMe->GetWeaponAmmo(WEAPON_GUN) != 10)
...
void CBot::MakeChoice(bool UseTarget)
{
	...
	TempChar.Tick(true, &TempWorld.m_Tuning);
	TempChar.Move(&TempWorld.m_Tuning);
	TempChar.Quantize();
