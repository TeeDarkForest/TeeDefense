...
void CGraph::ComputeClosestPath()
{
	...
	if(m_pClosestPath)
		free(m_pClosestPath);
	m_pClosestPath = (int*)malloc(m_NumVertices*m_NumVertices*sizeof(int));
	if(!m_pClosestPath)
		return;
	int* Dist = (int*)malloc(m_NumVertices*m_NumVertices*sizeof(int));
	if(!Dist)
		return;
...
void CBotEngine::GenerateSegments()
{
	...
	m_pSegments = (CSegment*) malloc(m_SegmentCount*sizeof(CSegment));
...
void CBotEngine::GenerateCorners()
{
	...
	m_pCorners = (vec2*)malloc(CornerCount*sizeof(vec2));
...
void CBotEngine::GenerateTriangles()
{
	...
	m_Triangulation.m_pTriangles = (CTriangulation::CTriangleData*)malloc(2*CornerCount*sizeof(CTriangulation::CTriangleData));
	vec2 *Corners = (vec2*)malloc((CornerCount+3)*sizeof(vec2));
	int m = 0;
	for(int i = 1;i < m_Width - 1; i++)
	{
		for(int j = 1; j < m_Height - 1; j++)
...
void CBotEngine::GenerateGraphFromTriangles()
{
	...
	m_Graph.m_pVertices = (CVertex*)malloc(m_Graph.m_NumVertices*sizeof(CVertex));
	m_Graph.m_pEdges = (CEdge*)malloc(m_Graph.m_NumEdges*sizeof(CEdge));
	for (int i = 0; i < m_Triangulation.m_Size; i++)
...
int CBotEngine::DistanceToEdge(CEdge Edge, vec2 Pos)
{
	...
	vec2 InterPos = closest_point_on_line_s(Edge.m_Start,Edge.m_End, Pos);
