...
CQian::CQian(CGameWorld *pGameWorld, int m_CoreID, int Owner, vec2 Pos) : CEntity(pGameWorld, CGameWorld::ENTTYPE_QIAN)
{
    dbg_msg("CREATE!", "GOOD");
    m_Pos = Pos;
    m_Owner = Owner;
...
CQian::CLaserEyes::CLaserEyes(CGameWorld *pGameWorld, int Type, CQian *OwnerQian) : CEntity(pGameWorld, CGameWorld::ENTTYPE_EYE)
{
...
void CQian::CLaserEyes::Tick()
{
    ...
    {
        vec2 TargetPos;
        CCharacter *Target = GameWorld()->IntersectCharacter(this->GetPos(), GetDirection(this->m_Degres), 32, TargetPos, GameServer()->GetPlayerChar(pOwnerQian->GetOwner()));
        if (Target)
        {
            new CDangerLaser(GameWorld(), this->GetPos(), TargetPos, Target->GetCID(), pOwnerQian->GetOwner());
        }
        else
        {
    ...
    dbg_msg("help", "%f, %f", GetDirection(this->m_Degres).x, GetDirection(this->m_Degres).y);
}
...
void CQian::CLaserEyes::Snap(int SnappingClient)
{
    ...
    if(m_BreathType == 0)
        m_Breath--;
    float Radius = 3.0f * 32.0f + m_Breath;
