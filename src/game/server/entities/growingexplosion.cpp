				...
				if(FromLeft || FromRight || FromTop || FromBottom)
				{
					...
					switch(m_ExplosionEffect)
					{
						...
						case GROWINGEXPLOSIONEFFECT_BOOM:
							...
							break;
						case GROWINGEXPLOSIONEFFECT_AbyssCore:
							if (random_prob(0.2f))
							{
								GameServer()->CreateHammerHit(vec2(TileCenter.x, TileCenter.y));
								GameServer()->CreateHammerHit(vec2(TileCenter.x, TileCenter.y+16));
								GameServer()->CreateHammerHit(vec2(TileCenter.x, TileCenter.y-16));
								GameServer()->CreateHammerHit(vec2(TileCenter.x+16, TileCenter.y));
								GameServer()->CreateHammerHit(vec2(TileCenter.x-16, TileCenter.y));
							}
							break;
						case GROWINGEXPLOSIONEFFECT_ELECTRIC:
